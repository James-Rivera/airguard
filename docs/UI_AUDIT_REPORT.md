...
| --- | --- | --- | --- |
| Bottom navigation | Bar felt oversized, could overlap content, and icons previously read like letter placeholders. | `src/components/ui/BottomTabBar.tsx`, `src/theme/spacing.ts`, `src/theme/shadows.ts` | Rebuilt as a safe-area-aware floating 72px pill with responsive width, real SVG icons, 82px active pill, softer Figma shadow, and larger screen bottom padding. |
